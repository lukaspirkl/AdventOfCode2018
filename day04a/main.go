...
func main() {
	...
	theBiggestSleeperID := findTheBiggestSleeper(guards)
	fmt.Println("-----")
	fmt.Println(theBiggestSleeperID)
	mostFrequentMinut := findMostFrequentSleepingMinute(guards[theBiggestSleeperID])
	fmt.Println(mostFrequentMinut)
	fmt.Println("-----")
	fmt.Println(theBiggestSleeperID * mostFrequentMinut)

}

func findMostFrequentSleepingMinute(sleeps []sleep) int {
...
func findTheBiggestSleeper(guards map[int][]sleep) int {
	longestID := 0
	longestDuration := 0
	for id, sleeps := range guards {
		fmt.Println(id)
		sum := 0
		for _, sleep := range sleeps {
			sum += int(sleep.duration.Minutes())
			fmt.Printf("\t%v\n", sleep.duration)
		}
		if longestDuration < sum {
			longestDuration = sum
			longestID = id
		}
	}
